...
void Game::initialize()
{
	...
	std::string shaderString = load_file("VertexShader.txt");
	const char* shaderCharacter = shaderString.c_str();

	DEBUG_MSG("Setting Up Vertex Shader");

	vsid = glCreateShader(GL_VERTEX_SHADER); //Create Shader and set ID
	glShaderSource(vsid, 1, (const GLchar**)&shaderCharacter, NULL); // Set the shaders source
	...
	std::string fragmentString = load_file("FragmentShader.txt");
	const char* fragmentCharacter = fragmentString.c_str();

	DEBUG_MSG("Setting Up Fragment Shader");

	fsid = glCreateShader(GL_FRAGMENT_SHADER);
	glShaderSource(fsid, 1, (const GLchar**)&fragmentCharacter, NULL);
...
void Game::update()
{
	...
	{
		matrixApplicationFunction(MyMatrix3::rotationY(0.0005));
	}
	...
	{
		matrixApplicationFunction(MyMatrix3::rotationX(0.0005));
	}
...
std::string Game::load_file(const std::string & src)
{
	ifstream file;
	file.open(((src).c_str()));
	string output;
